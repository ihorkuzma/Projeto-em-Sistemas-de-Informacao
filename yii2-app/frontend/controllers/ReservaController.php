...
class ReservaController extends Controller
{
    ...
    public function actionView($id)
     {
         $Reserva = Reserva::findAll(['Cliente_id'=>$id]);
    ...
    public function actionCreate()
    {
        ...
        $reserva = Yii::$app->request->post('Reserva');

        $model->Data_reserva= $reserva['Data_reserva'];


        if ($model->save()){


            return $this->redirect(['alimento/index2', 'id' => $model->id]);
        }
    ...
    public function actionPedido($id,$qtd,$Preco){

        $model= Alimento::findOne(['id' => $id]);
        $alimentos = Alimento::find() -> all();
        $vendas= Vendas::find() ->all();
        ...
        $vendas->Cliente_id = Yii::$app->user->identity->id;
        $vendas->Alimento_id = $model->id;
        $vendas->Tipo_venda_id= 1;
        $vendas->id_mesa=0;

        return $this->render('index', ['total' => $total,'alimentos' => $alimentos, 'vendas' => $vendas]);
